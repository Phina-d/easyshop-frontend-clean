...
export default function UsersListAdmin() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      console.log("✅ Utilisateurs chargés :", res.data); // <-- Ajout temporaire
      setUsers(res.data);
    } catch (error) {
  ...
  // Filtrer la liste complète selon searchTerm (nom, email, téléphone)
  ...
  // Trier les utilisateurs filtrés par rôle
  const admins = filteredUsers.filter((u) => u.role === "admin");
  const closers = filteredUsers.filter((u) => u.role === "closer");
  const chefs = filteredUsers.filter((u) => u.role === "chef_service");
